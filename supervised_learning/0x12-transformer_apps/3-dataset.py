...
class Dataset:
    ...
    def __init__(self, batch_size, max_len):
        """ batch_size: batch size for training/validation
            max_len: the maximum number of tokens per example sentence """
        ...
        b_size = info.splits['train'].num_examples
        self.data_train = self.data_train.filter(
            lambda i, j: tf.math.logical_and(
                tf.size(i) <= self.max_len, tf.size(j) <= self.max_len))
        self.data_train = self.data_train.cache().shuffle(b_size).padded_batch(
        ...
        self.data_valid = self.data_valid.filter(
            lambda m, n: tf.math.logical_and(
                tf.size(m) <= self.max_len, tf.size(n) <= self.max_len))
